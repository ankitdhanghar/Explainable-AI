...
from train import ( train_rf, train_xgb,build_ann, train_ann)
...
for model_name in config.models:
    ...
    # XGB
        ...
        metrics = evaluate_model(model,model_name,X_test,y_test,config)
    ...
    print("after")
    if model_name == 'ANN':
        plot_shap_summary(shap_values=shap_values, X_test=X_shap, config=config, model_name=model_name)
        plot_shap_beeswarm(shap_values=shap_values,config=config,model_name=model_name)
        plot_shap_dependence(shap_values=shap_values,X_test=X_shap,feature_name=X_shap.columns[0],config=config,model_name=model_name)
        plot_shap_waterfall(shap_values=shap_values,sample_index=0,config=config,model_name=model_name)
        plot_shap_force( explainer=shap_explainer, shap_values=shap_values, X_test=X_shap, sample_index=0, config=config, model_name=model_name)
        plot_shap_interaction(shap_values=shap_values,X_test=X_shap,feature_name=X_shap.columns[0],config=config, model_name=model_name)
    else:
